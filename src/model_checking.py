...
def check_entailment(kb: list[Formula], query: Formula) -> bool:
    ...
    for f in kb:
        all_atoms |= get_atoms(f)
    all_atoms |= get_atoms(query)
    for model in get_all_models(all_atoms):
        if all(evaluate(f, model) for f in kb) and not evaluate(query, model):
            return False
    return True
...
def truth_table(formula: Formula) -> list[tuple[dict[str, bool], bool]]:
    ...
    rows = []
    for model in get_all_models(get_atoms(formula)):
        rows.append((model, evaluate(formula, model)))
    return rows
